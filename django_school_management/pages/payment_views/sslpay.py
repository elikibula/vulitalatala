...
# Check if settings.USE_PAYMENT_OPTIONS is True before importing SSLCOMMERZ and settings attributes.
...
def online_admission_sslpayment(request, pk):
    ...
    # Only create SSLCOMMERZ instance if USE_PAYMENT_OPTIONS is True
    if settings.USE_PAYMENT_OPTIONS:
        ssl_settings = {
    ...
    # Only create session if USE_PAYMENT_OPTIONS is True
    if settings.USE_PAYMENT_OPTIONS:
    ...
    # Return an empty response if USE_PAYMENT_OPTIONS is False
    return HttpResponse()
